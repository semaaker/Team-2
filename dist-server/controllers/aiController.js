...
import { aiProvider, analyzeEvents, criteriaFromSponsor, scoreEvent, } from '../services/aiMatching.js';
/**
 * Yapay zeka uçları.
 *
 ...
 */
export function resolveCriteria(user) {
    if (!user)
        return criteriaFromSponsor(undefined);
...
function applyOverrides(criteria, body) {
    return {
        ...criteria,
...
function toMatchItem(event, score, note, breakdown) {
    return {
        id: `mth_${event.id}`,
        eventId: event.id,
 ...
 */
export async function buildRankedMatches(user, options) {
    ...
    let items = candidates.map((event) => {
        ...
        return toMatchItem(event, analysis?.score ?? event.aiMatchScore, analysis?.note ?? event.aiNote, analysis?.breakdown);
    });
    if (options.category) {
        items = items.filter((item) => matches(item.industry, options.category));
    }
    items.sort((a, b) => {
        if (options.sort === 'date')
            return a.eventName.localeCompare(b.eventName, 'tr');
...
export const aiController = {
     ...
     */
    async analyzeEvent(req, res) {
        const event = db.events.find((item) => item.id === req.params.id);
        if (!event)
            throw ApiError.notFound('Bu etkinlik bulunamadı.');
        ...
        if (criteria.sponsorId) {
            ...
            if (!analysis)
                throw ApiError.internal('Yapay zeka değerlendirmesi üretilemedi.');
            return res.json({
                eventId: event.id,
        ...
        if (pool.length === 0)
            throw ApiError.internal('Değerlendirilecek sponsor profili yok.');
        ...
        res.json({
            ...
            meta: { ...aiProvider(), source: 'local', generatedAt: new Date().toISOString() },
        });
    },
};
