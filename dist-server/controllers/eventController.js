...
export const eventController = {
    ...
    list(req, res) {
        ...
        if (search) {
            results = results.filter((event) => matches(event.name, search) ||
    ...
    create(req, res) {
        ...
        const event = {
            ...
            attendeesLabel: payload.participantCount.replace(/(\d+)\+?/, (_m, n) => `${Number(n).toLocaleString('tr-TR')}+`),
            attendees,
            coverImageUrl: '',
            status: 'seeking',
    ...
    toggleBookmark(req, res) {
        ...
        if (!event)
            throw ApiError.notFound('Etkinlik bulunamadı.');
        const set = db.bookmarks.get(req.user.id) ?? new Set();
        if (set.has(event.id))
            set.delete(event.id);
        else
            set.add(event.id);
    ...
    updateProposal(req, res) {
        ...
        if (!proposal)
            throw ApiError.notFound('Teklif bulunamadı.');
