...
export const sponsorController = {
  ...
  sponsorships(req: Request, res: Response) {
  ...
  milestones(_req: Request, res: Response) {
  ...
  deal(req: Request, res: Response) {
    ...
    if (!deal) throw ApiError.notFound('Bu anlaşma bulunamadı.');
  ...
  toggleDeliverable(req: Request, res: Response) {
    const deal = db.deals.find((item) => item.id === req.params.id);
    if (!deal) throw ApiError.notFound('Bu anlaşma bulunamadı.');

    const deliverable = deal.deliverables.find((item) => item.id === req.params.deliverableId);
    if (!deliverable) throw ApiError.notFound('Teslimat kalemi bulunamadı.');
  ...
  profile(req: Request, res: Response) {
    ...
    if (!sponsor) throw ApiError.notFound('Sponsor profili bulunamadı.');
