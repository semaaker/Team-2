...
import type { StatMetric, SupportTicket } from '../types.js';
...
export const contentController = {
  ...
  legal(req: Request, res: Response) {
    const doc = db.legalDocs.find((item) => item.slug === req.params.slug);
    if (!doc) throw ApiError.notFound('Belge bulunamadı.');
  ...
  createTicket(req: Request, res: Response) {
    const payload = req.body as z.infer<typeof supportTicketSchema>;
  ...
  platformStats(_req: Request, res: Response) {
  ...
  organizerStats(req: Request, res: Response) {
  ...
  sponsorStats(_req: Request, res: Response) {
