...
export const settingsController = {
  ...
  updateProfile(req: Request, res: Response) {
    ...
    if (payload.fullName !== undefined) req.user.fullName = payload.fullName;
    if (payload.companyName !== undefined) req.user.companyName = payload.companyName;
    if (payload.title !== undefined) req.user.title = payload.title;
    if (payload.phone !== undefined) req.user.phone = payload.phone;
    if (payload.avatarUrl !== undefined) req.user.avatarUrl = payload.avatarUrl;

    const { fullName, email, companyName, title, phone, avatarUrl } = req.user;
    res.json({ fullName, email, companyName, title, phone: phone ?? '', avatarUrl });
  },

  notifications(req: Request, res: Response) {
    if (!req.user) throw ApiError.unauthorized();
  ...
  updateNotifications(req: Request, res: Response) {
    if (!req.user) throw ApiError.unauthorized();

    initUserSettings(req.user.id);
    const current = db.notificationSettings.get(req.user.id);
    if (!current) throw ApiError.notFound('Bildirim tercihleri bulunamadı.');

    const patch = req.body as z.infer<typeof notificationSchema>;
  ...
  security(req: Request, res: Response) {
    if (!req.user) throw ApiError.unauthorized();
  ...
  setTwoFactor(req: Request, res: Response) {
    if (!req.user) throw ApiError.unauthorized();

    initUserSettings(req.user.id);
    const settings = db.securitySettings.get(req.user.id);
    if (!settings) throw ApiError.notFound('Güvenlik ayarları bulunamadı.');

    const { enabled } = req.body as z.infer<typeof twoFactorSchema>;
  ...
  revokeSession(req: Request, res: Response) {
    ...
    if (!session) throw ApiError.notFound('Oturum bulunamadı.');
